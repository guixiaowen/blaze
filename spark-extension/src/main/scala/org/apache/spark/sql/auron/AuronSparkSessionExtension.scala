...
import org.apache.spark.sql.execution.{ColumnarRule, FileSourceScanExec, LeafExecNode, SparkPlan}
  ...
  override def preColumnarTransitions: Rule[SparkPlan] = {
    new Rule[SparkPlan] {
      override def apply(sparkPlan: SparkPlan): SparkPlan = {
        if (sparkPlan.isInstanceOf[LeafExecNode] && !sparkPlan.isInstanceOf[FileSourceScanExec]) {
          return sparkPlan
        }
